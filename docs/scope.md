...
Ten landing-page design studies are implemented, using Next.js App Router, TypeScript, Tailwind CSS, and Bun. Five come from [astra](../astra/README.md) and five from [opus](../opus/README.md); opus was written without reading the astra directory, so the two sets are independent proposals. The opus design reasoning and claim boundaries live in its [design plan](../opus/docs/design-plan.md).

One application at the repository root serves all ten and is the only thing that builds. It owns the routes, the shared overview at `/`, and the one switcher that moves between any two designs. Each model keeps its own page code, stylesheet, and assets in its own directory, and each stylesheet is scoped so that no set can style another: astra rules sit under `.astra`, opus rules under `.d1` to `.d5`. Routes are `/astra/one` to `/astra/five` and `/opus/one` to `/opus/five`, so the model that built a page is visible in its address.
